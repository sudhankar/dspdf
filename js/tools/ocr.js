...
(function () {
  ...
  var state = { file: null, bytes: null, pdfDoc: null, pageCount: 0, lastText: "" };
  var progressUI = null;
    ...
    try {
      ...
      for (var k = 0; k < pageOutputs.length; k++) {
        ...
        po.words.forEach(function (w) {
          ...
          try {
            var scaleX = po.pageW / 1000; // heuristic — Tesseract bbox in pixels at unknown scale
            // We use a normalised approach: divide bbox by assumed render scale
            // Simplest: rely on fontSize from bbox height scaled to page
            var fontSize = Math.max(4, (w.bbox.y1 - w.bbox.y0) * (po.pageH / 1000) * 0.9);
            if (!isFinite(fontSize) || fontSize < 3) return;
            page.drawText(w.text, {
              x: (w.bbox.x0 / 1000) * po.pageW,
              y: po.pageH - (w.bbox.y1 / 1000) * po.pageH,
              size: fontSize,
              font: font,
